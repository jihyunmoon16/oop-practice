...
public class PercentDiscountMembershipLevelTest {
    private Customer c1;
    private Customer c2;
    private Customer c3;
    ...
    @Test
    @DisplayName("골드레벨 이상 퍼센트할인 테스트")
    public void testPercentDiscountMembershipLevelAboveGold() {
        discount10percent = new PercentDiscountPolicy(0.10, new MembershipLevelCondition(MembershipLevel.GOLD));

        cartC1 = new Cart(c1, discount10percent);
        cartC2 = new Cart(c2, discount10percent);
        cartC3 = new Cart(c3, discount10percent);
        ...
        assertEquals(cartC2.calculateFinalCost(), Money.wons(45000));
        assertEquals(cartC3.calculateTotalCostOfItemsInCart(), Money.wons(50000));
        assertEquals(cartC3.calculateFinalCost(), Money.wons(45000));
    }

    @Test
    @DisplayName("VIP레벨 이상 퍼센트할인 테스트")
    public void testPercentDiscountMembershipLevelAboveVIP() {
        discount10percent = new PercentDiscountPolicy(0.1, new MembershipLevelCondition(MembershipLevel.VIP));
